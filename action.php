...
function requireCsrfToken() {
	if (empty($_POST['csrf_token']) || empty($_SESSION['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
		sendJsonResponse(['error' => 'Invalid request token'], 403);
	}
}

$writeActions = array(
	'customerAdd', 'customerUpdate', 'customerDelete',
	'categoryAdd', 'updateCategory', 'deleteCategory',
...
if (!empty($_POST['btn_action']) && in_array($_POST['btn_action'], $writeActions, true)) {
	requireCsrfToken();
}

if (!empty($_POST['logout_action']) && $_POST['logout_action'] === 'logout') {
	requireCsrfToken();
	session_unset();
	session_destroy();
	redirect('index.php');
